...
#Region " Web Form Designer Generated Code "
    ...
    Protected WithEvents ucCentroCusto As wucCentroCusto
    Protected WithEvents lnkFecharTelaPago As System.Web.UI.WebControls.LinkButton
    Protected WithEvents lnkFecharTelaPerda As System.Web.UI.WebControls.LinkButton
...
#Region " Eventos "
    ...
    Private Sub btnPesquisar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnPesquisar.Click
            ...
            Else
                If (ucFornecedor.CodFornecedor > 0) Then
                    DatasetAcordo1 = Controller.ObterAcoesComerciaisBaixa( _
                        ...
                        3)
                Else
                    Util.AdicionaJsAlert(Me, "Informe os parâmetros de Indicação do Acordo desejados para a pesquisa")
                    Exit Sub
                End If
            End If
        End If
        ...
        If Not encontradoRegistros Then
            ...
            Exit Sub
        End If
        ucCentroCusto.ReinicializaControle()
    ...
    Private Sub lnkFecharTelaPago_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lnkFecharTelaPago.Click
        Try
            cmdImprimir.Enabled = True
            btnGravar.Enabled = False
            LimparTela()
    ...
    Private Sub lnkFecharTelaPerda_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles lnkFecharTelaPerda.Click
        Try
            LimparTela()
        Catch ex As Exception
            Controller.TrataErro(Me.Page, ex)
    ...
    Private Sub txtCODPMS_ValueChange(ByVal sender As System.Object, ByVal e As Infragistics.WebUI.WebDataInput.ValueChangeEventArgs) Handles txtCODPMS.ValueChange
        Try
            txtfilial.Text = String.Empty
            LimparTela()
            If Not txtCODPMS Is String.Empty Then
                Dim dsAcordo As wsAcoesComerciais.DatasetAcordo
                Dim CODPMS As Decimal = 0
                CODPMS = txtCODPMS.ValueDecimal
                If CODPMS = 0 Then Exit Sub

                dsAcordo = Controller.ObterAcordos(0, 0, 0, CODPMS, 0, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, String.Empty, 0)

                If dsAcordo.T0118058.Rows.Count = 0 Then
                    Util.AdicionaJsAlert(Me.Page, "Acordo não encontrado")
                    txtCODPMS.Text = String.Empty
                    Exit Sub
                End If

                Dim CODFILEMP As Decimal
                CODFILEMP = dsAcordo.T0118058(0).CODFILEMP

                If Not cmbCODFILEMP.Items.FindByValue(CODFILEMP.ToString) Is Nothing Then
                    cmbCODFILEMP.SelectedValue = CODFILEMP.ToString
                    txtfilial.Text = cmbCODFILEMP.SelectedItem.ToString
                Else
                    cmbCODFILEMP.Items.Insert(0, New ListItem("Filial: " & CODFILEMP.ToString & " NÃO CADASTRADA!", CODFILEMP.ToString))
                    cmbCODFILEMP.SelectedValue = CODFILEMP.ToString
                    txtfilial.Text = cmbCODFILEMP.SelectedItem.ToString
                End If

                txtNUMPEDCMP.Text = String.Empty
    ...
    Private Sub txtNUMPEDCMP_ValueChange(ByVal sender As System.Object, ByVal e As Infragistics.WebUI.WebDataInput.ValueChangeEventArgs) Handles txtNUMPEDCMP.ValueChange
        Try
            txtfilial.Text = String.Empty
            LimparTela()
            If Not txtNUMPEDCMP.Value Is String.Empty Then
                Dim dsAcordo As wsAcoesComerciais.DatasetAcordo
                Dim NUMPEDCMP As Decimal = 0
                NUMPEDCMP = CType(txtNUMPEDCMP.Text, Decimal)
                If NUMPEDCMP = 0 Then Exit Sub

                dsAcordo = Controller.ObterAcordos(0, 0, 0, 0, 0, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, Nothing, String.Empty, NUMPEDCMP)

                If dsAcordo.T0118058.Rows.Count = 0 Then
                    Util.AdicionaJsAlert(Me.Page, "Pedido não encontrado")
                    txtNUMPEDCMP.Text = String.Empty
                    Exit Sub
                End If
...
#Region " Métodos de Regras de Negócio "
        ...
        Try
            ...
            If VlrLim >= txtValorPerda.ValueDecimal Then
                ...
                For Each linha As wsAcoesComerciais.DatasetAcordo.T0118066Row In DatasetAcordo1.T0118066
                    ...
                    End If
                Next
                'TIPO DE ACORDO DSC INIBIDO
                If linhaT0118066.TIPFRMDSCBNF = 2 And Not uwgPesquisa.DisplayLayout.ActiveRow.Cells(1).Value Is Nothing Then
                    Util.AdicionaJsAlert(Me, "Operação indisponível para acordos com forma de pagamento 'DESCONTO DUPLICATA'!")
                    Exit Sub
                End If
                'Atualiza os valores
                ...
                If ucCentroCusto.CodCentroCusto > 0 Then
                    linhaT0120540.CODCENCST = Convert.ToInt32(ucCentroCusto.CodCentroCusto)
                Else
                    linhaT0120540.SetCODCENCSTNull()
                End If
                Try
                ...
                WSCAcoesComerciais.T0120540Row = linhaT0120540
                WSCAcoesComerciais.T0118066Row = linhaT0118066
                Page.RegisterStartupScript("Selecao", "<script>CarregarJanelaObservacaoBaixaAcordoPerda();</script>")
            Else
    ...
    Private Sub gravar()
        Try
            If txtValorPago.ValueDecimal = 0 Then
                Util.AdicionaJsAlert(Me, "Favor preencher o campo Valor Pago.")
                Exit Sub
            End If
            ...
            For Each linha As wsAcoesComerciais.DatasetAcordo.T0118066Row In DatasetAcordo1.T0118066
                'Localiza a linha que deverá ser atualizada
                If linha.TIPFRMDSCBNF = Convert.ToDecimal(txtFormaPagto.Text.Split(Convert.ToChar("-"))(0).Trim()) And _
                    linha.TIPDSNDSCBNF = Convert.ToDecimal(txtDestinoVerba.Text.Split(Convert.ToChar("-"))(0).Trim()) And _
                    linha.DATPRVRCBPMS = txtDataPrevisaoRecebimento.Date Then
                    If linha.IsINDASCARDFRNPMSNull Then
                        linhaT0118066 = linha
                        Exit For
            ...
            WSCAcoesComerciais.T0118066Row = linhaT0118066
            WSCAcoesComerciais.T0120540Row = linhaT0120540
            Page.RegisterStartupScript("Selecao", "<script>CarregarJanelaObservacaoBaixaAcordoPago();</script>")
        Catch ex As Exception
            Controller.TrataErro(Me.Page, ex)
        End Try
    End Sub
